...
async function fetchPokemon(pokemonName = null) {
    try{
        ...
        const pokemonInput =pokemonName ||document.getElementById("searchInput").value.toLowerCase();
...
//         const responce = await fetch(url);
//         if(!responce.ok){
//             throw new Error("Pokemon not found in speciesfetch");
//         }
//         const data = await responce.json();
...
//         const pokemonContainer = document.getElementById("pokemonContainer");
//         pokemonContainer.style.display = "flex";
//         pokemonContainer.textContent = "Pokemon not found. Please try again. from 2nd";
//     }
// }
...
// async function fetchEvolutionImage(names) {
...
//             imgElement.alt = name;
//             pElement.classList.add("pokemonName");
//             pElement.textContent = `#${data.id}- ${name} `;
            
//             evolutionChainContainer.appendChild(imgElement);
//             evolutionChainContainer.appendChild(pElement);
...
// function printEvolution(chain){
//     evolution_chain.push(chain.species.name)
//     chain.evolves_to.forEach(evolution => {
//         printEvolution(evolution);
//     });
// }
...
function displayPokemon(data){
    ...
    pokemonAbilitys.innerHTML = "Abilities:";
    ...
    data.stats.forEach(stat => {
        ...
        statElement.classList.add("progress-bar");
        statElement.style.width = `${(stat.base_stat / 400) * 100}%`;
